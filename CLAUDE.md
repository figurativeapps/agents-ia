...
- **Phase 2 (COMPLETE→IN_PROGRESS)**: Admin fills "lien ra" with AR link, "Titre snapshot" with overlay title, attaches snapshot.png + qrcode.png, sets subtask to COMPLETE. Script generates PDF (overlay_pdf with "Titre snapshot" as title), uploads to R2, creates HubSpot note (with AR link + PDF link), sets `hs_lead_status=IN_PROGRESS`, and clears `clickup_prospection_task_id`.
- **Re-trigger**: Remettre `hs_lead_status=OPEN`. Comme `clickup_prospection_task_id` a été effacé après Phase 2, le script traite le contact comme nouveau et crée une nouvelle subtask (retour en Phase 1). Si la subtask a été supprimée dans ClickUp (`get_task_full` retourne None), même comportement.
- **Cleanup**: `execution/cleanup_prospection.py` — script one-shot pour supprimer les subtasks dupliquées et corriger les `clickup_prospection_task_id` sur HubSpot. Usage: `--dry-run` pour preview, sans flag pour appliquer.
